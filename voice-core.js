...
function baseVoiceState(){
  return {
    ...
    settings:{businessName:'Kelo Associates',defaultAgent:'agent_kelo',languages:['en','es'],humanTransferEnabled:true,autoBook:true,minimumHotScore:75},
...
export function ensureVoiceState(state){
  ...
  state.voice.settings={...baseVoiceState().settings,...(state.voice.settings||{})};
...
export class VoiceToolRouter{
  ...
  execute(name,args={},context={}){if(typeof this[name]!=='function'||name==='execute')throw new Error(`Tool no permitido: ${name}`);return this[name](args,context)}
  ...
  qualify_lead({clientId,vertical='bathroom',homeowner=null,budget=0,timelineDays=999,zip='',projectType=''}={},ctx={}){
    ...
    let opportunity=this.state.opportunities.find(o=>o.clientId===clientId&&o.stage!=='won');
    if(!opportunity){opportunity={id:uid('o'),clientId,vertical,estimatedValue:Number(budget||0),stage:temp==='hot'?'qualified':'new',source:'Kelo Voice',createdAt:new Date().toISOString()};this.state.opportunities.push(opportunity)}else{opportunity.vertical=vertical;opportunity.estimatedValue=Number(budget||opportunity.estimatedValue||0);opportunity.stage=temp==='hot'?'qualified':opportunity.stage}
  ...
  create_appointment({clientId,vertical='bathroom',dateTime,location='Virtual consultation',estimatedValue=0,notes='Agendada por Kelo Voice'}={},ctx={}){
...
export function startVoiceCall(state,{name='Caller',phone,language='auto',vertical='bathroom',direction='inbound'}={}){
...
export function getVoiceMetrics(state){
  ensureVoiceState(state);const today=isoDay(0);const calls=state.voice.calls.filter(c=>String(c.startedAt||'').slice(0,10)===today);const completed=calls.filter(c=>c.status==='completed');const booked=completed.filter(c=>c.outcome==='appointment_booked');const handled=completed.filter(c=>c.outcome!=='human_transfer');const hot=completed.filter(c=>Number(c.leadScore||0)>=state.voice.settings.minimumHotScore);return {calls:calls.length,completed:completed.length,handled:handled.length,booked:booked.length,hot:hot.length,resolutionRate:completed.length?Math.round((handled.length/completed.length)*100):0};
}

export {STORAGE,VOICE_SCHEMA};
