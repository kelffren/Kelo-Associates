...
function centralInventorySelection(){const form=$('#centralInventoryForm'),vertical=form.elements.vertical.value,variantKey=String(form.elements.variantKey.value||'default').trim()||'default',product=getCatalog(vertical)[0];$('#centralSku').value=product?.sku||'';return {vertical,variantKey,sku:product?.sku||'',quantity:Math.max(0,Math.floor(Number(form.elements.quantity.value||0)))}}
...
$('#voiceDemoForm').addEventListener('submit',async event=>{
  ...
  const live=getBackendStatus().ready,product=getCatalog(input.vertical)[0];
  try{
    ...
    }else if(result.order?.status==='pending_payment'&&!live)status+=' Backend aún no conectado: no se envió nada real.';
